...
func rippleDataTest(tranNum int) {
	...
	idMap := dataproc.ConvertToSeriesID(channelAssignWay, g, 0)
...
// flash的实验方式，循环删除度为1的结点，并且是在提前删除边的值为0的前提下，所以得到的结点比较少
func rippleSnapShotDataFlashTest(tranNum int) {
	channelAssignWay := dataproc.REBALANCE_CHANEL
	transSampleWay := dataproc.REMAINDER_SAMPLE
	logName := fmt.Sprintf("-rippleSnapshot-flashtopo-%s-%s",
		testNameMap[channelAssignWay], testNameMap[transSampleWay])
...
func lightningDataTest(tranNum int) {
	...
	dataproc.ConvertToSeriesID(dataproc.ORIGION_CHANNEL, g, 0)
...
// 为了debug对比flash的实验结果，将transaction和结点的顺序都固定
func debugRippleSnapShotDataTest(tranNum int) {
	...
	dataproc.ConvertToSeriesIDWithMap(channelAssignWay, g, idMap, 0)
